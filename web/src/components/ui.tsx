...
export function Input({
  ...
  return (
    <div className="relative w-full">
      <input
        className={`w-full rounded-lg border border-line bg-white px-3 py-2 text-sm outline-none ring-accent/20 focus:border-accent focus:ring-2 ${
          showClear ? "pr-8" : ""
        } ${className}`}
        value={value}
        disabled={disabled}
...
export function TableScroll({
  ...
  return (
    <div
      className={`max-h-[min(62vh,calc(100vh-260px))] overflow-auto overscroll-contain [&_thead_th]:sticky [&_thead_th]:top-0 [&_thead_th]:z-10 [&_thead_th]:bg-soft [&_thead_th]:shadow-[0_1px_0_var(--line)] ${className}`}
    >
      {children}
    </div>
  );
